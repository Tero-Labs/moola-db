CREATE OR REPLACE FUNCTION public.func_getexchange_rate_new()
...
BEGIN
    ...
    select usd_exchange_rate,cusd_exchange_rate,ceuro_exchange_rate into celo_to_usd,celo_to_cusd,celo_to_ceuro
	from tbl_coin_exchange_rate 
	where coin_name = 'Celo' and enabled=true
	order by  creation_datetime desc limit 1;
	
	select usd_exchange_rate,ceuro_exchange_rate,celo_exchange_rate into cusd_to_usd,cusd_to_ceuro,cusd_to_celo
	from tbl_coin_exchange_rate 
	where coin_name = 'cUSD' and enabled=true
	order by  creation_datetime desc limit 1;

	select usd_exchange_rate,cusd_exchange_rate,celo_exchange_rate into ceuro_to_usd,ceuro_to_cusd,ceuro_to_celo
	from tbl_coin_exchange_rate 
