CREATE OR REPLACE FUNCTION public.func_getexchange_rate_ceur_to_x_nearest_past_t(in_currency_convert_to_type character varying, in_value numeric, in_block_no numeric)
 RETURNS numeric
 LANGUAGE plpgsql
AS $function$
declare
...
BEGIN
   ...
   --- need a set code for missing IN/OUT currency & setting it to 1 may be using NOT EXISTS()
