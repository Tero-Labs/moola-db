CREATE OR REPLACE FUNCTION public.func_getexchange_rate_celo_to_x_t(in_currency_convert_to_type character varying, in_value numeric)
 RETURNS numeric
 LANGUAGE plpgsql
AS $function$
declare
...
BEGIN
	...
	order by block_number desc limit 1;
    -- and enabled=true
    --- Need to add handles if in/out currenct missing like - not exists etc
